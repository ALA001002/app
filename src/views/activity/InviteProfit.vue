<template>
  <div class="invite-profit">
    ...
    <div class="yaoqing">
      ...
      <div class="yaoqnum">
        <div class="invitenum"><span>{{$t('activity.invitenum')}}</span></div>
        <div><span>{{info.invitedNumberCount}}{{$t('activity.invitepen')}}</span></div>
      </div>
    </div>
    <div class="yaolist">
...
export default {
  ...
  methods: {
    init() {
      this.$http.api.get(`asset/inviteDetail?timeStatus=${this.current}&coin=PPB`).then(data => {
        this.info = data.data
      })
    },
    change(val) {
