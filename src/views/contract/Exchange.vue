  ...
  <div class="hero is-full has-text-white has-background-grey-darker is-size-7">
    ...
    <van-dialog v-model="showConfirm" :title="$t('exchange.confirmBuy')" class="dark-dialog" show-cancel-button @confirm="sub">
      <div class="confirmPanel">
        ...
        <div>{{$t('exchange.trustType')}}</div><div>{{form.trustType == '0' ? $t('exchange.marketPrice') : $t('exchange.trigger')}}</div>
        <div v-show="form.trustType == '1'">{{$t('exchange.triggerPrice')}}</div><div v-show="form.trustType == '1'">{{form.triggerPrice}}</div>
        <div>{{$t('exchange.margin')}}</div><div>{{form.amount}} <small>{{form.currency}}</small></div>
        <div>{{$t('order.stopSurplus')}}</div><div>{{stopSurplus}}</div>
        <div>{{$t('order.stopLoss')}}</div><div>{{stopLoss}}</div>
      </div>
    </van-dialog>
  </div>
</template>
...
export default {
  ...
  components: { HeadSafeArea, TextFormat, KLine, CollapseTransition, ContractForm },
  data() {
    return {
      showBar: false,
      showPanel: false,
  ...
  mounted() {
    ...
    this.$refs.contractForm.$el.addEventListener(window.ontransitionend ? 'transitionend' : 'webkitTransitionEnd', () => {
