<template>
  <div class="subscribe">
    <div class="card">
      ...
      <div class="top1">
        <span style="margin-right: 20px;">PPB</span>
        <span>{{info.currentPrice}}</span>
        <span style="font-size: 10px;">PPB/USDT</span>
      </div>
      <div class="top2">
        {{$t('activity.subscribeyg')}}
      </div>
      <div class="top1">
        {{info.exchangeNum}} PPB
      </div>
...
export default {
  ...
  mounted() {
    this.$http.api.get('asset/exchangeRecord?coin=PPB').then(data => {
      this.info = data.data
    })
