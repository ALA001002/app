<template>
  <div>
    <BackPanel style="background: var(--btn-main-bg);color:var(--text-deep-color)" :title="$t('spot.spotOrder')">
      <template slot="right">
...
export default {
  ...
  methods: {
    ...
    cancelOrder(orderId) {
      this.$http.post('/ico/revoke', {orderIds: [orderId]}).then(
          () => {
            Toast.success()
...
.spot-btns div:last-child {
  background: #2ebd85;
}
.symbol-select > span.active {
  color: #f2a603!important;
}
.symbol-select > span.active .van-icon {
  color: #f2a603!important;
}
</style>
