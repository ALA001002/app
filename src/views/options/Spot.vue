...
<!--        <TextFormat :float="+(data && data.change)" class="text-12px ml-1" :text="+(data && data.change)" isFormat />-->
        <div class="symbol-list" style="display: flex;align-items: center">
          <van-icon name="fanhui" class-prefix="icon" class="has-text-grey-dark mr-2" @click="$router.back()" />
          <img src="@/assets/images/exchange/exchange1.png" @click="emitChange" style="height: 33px;margin-right: 10px"/>
      ...
      <div class="body">
        ...
        <div class="right">
          <div class="tabs">
            <span @click="()=>buyIndex=0" :class="{active:buyIndex==0}">{{ $t('spot.buy') }}</span>
            <span @click="()=>buyIndex=1" :class="{active:buyIndex==1,sell:buyIndex==1}">{{ $t('spot.sell') }}</span>
          </div>
...
<!--               />-->
                 <span class="iconfont1 icon-arrow-down-filling"></span>
               </div>
...
<!--              </select>-->
          ...
          <div class="price item" v-if="typeIndex==0">
            <div class="tab-change" v-if="buyIndex===0">
          ...
          <div class="price item" v-if="typeIndex==1">
            <van-field v-model="buyPrice">
              <template slot="left-icon">
                <span  @click="subPrice" class="iconfont1 icon-minus-bold"></span>
              </template>
              <template slot="right-icon">
                <span  @click="addPrice" class="iconfont1 icon-add"></span>
              </template>
            </van-field>
          </div>
          <div class="num item">
            <van-field type="number" v-model="buyNum"  @input="(e)=>{changeBuyNum(e)}" :placeholder="getPlaceHolder()">
              <template slot="left-icon">
                <span  @click="sub"  class="iconfont1 icon-minus-bold"></span>
              </template>
              <template slot="right-icon">
                <span @click="add" class="iconfont1 icon-add"></span>
              </template>
            </van-field>
          </div>
          <div class="percents">
  ...
  watch:{
    ...
    wallet(){
      for(const key in this.wallet){
        ...
        if(item.currency===type.toUpperCase()){
  ...
  methods:{
    ...
    queryDeep(){
      ...
      this.timer = setTimeout(()=>{
        self.$http.api.get(`/common/deepInfo/${self.code.toLowerCase()}`).then(({data}) => {
