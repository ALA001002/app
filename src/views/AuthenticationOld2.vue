  ...
  <BackPanel :title="$t('app.auth')" >
    <van-form @submit="onSubmit">
      ...
      <van-field name="radio" :label="$t('app.auth_type')">
        <template #input>
          <van-radio-group v-model="form.authType" direction="horizontal">
            ...
            <van-radio name="2" style="color:var(--text-deep);">{{$t('app.passport')}}</van-radio>
      ...
      <van-field :disabled="disable" :label="$t('app.idCardImg'+type)">
        <template #input>
          <van-uploader :after-read="afterRead" v-model="files" :disabled="disable" multiple :max-count="3" :before-read="beforeRead"/>
        </template>
      </van-field>
...
export default {
  ...
  methods: {
    onSubmit() {
      const files = this.files.map(item=>item.url)
