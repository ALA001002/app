<template>
  <van-popup v-model="showPlace" closeable round position="bottom" style="background:var(--btn-main-bg)" get-container="body">
    ...
    <div class="section overflow-auto" style="height:90vh">
      ...
      <template v-else-if="type==='2'">
        <div class="flex-between mb-3 mt-1">
          ...
          <span class="text-18px text-success">{{item.profitTime}} {{$t('extra.hour')}}</span>
        </div>
        <div class="flex-between mb-3 text-14px">
      ...
      <div class="radius-small p-2 flex-between" style="background:#28282A;color:white">
      ...
      <div class="mid-btn radius mt-5" :disabled="!number" @click="submit">{{$t('extra.pledgeNow')}}</div>
    </div>
  </van-popup>
</template>
