<template>
  <div class="section flex-y-full">
    <div class="flex text-shallow text-11px mt-0 mb-4">{{$t('defi.showAvailable')}}
      ...
      <div class="item mt-5" v-for="item in list" :key="item.id">
        <template v-if="(showAvailable && item.remainingNumber>=0) || !showAvailable">
          ...
          <div class="flex-between mt-4">
            <span class="text-13px text-mid ">{{$t('defi.annualBenchmarkYield')}}</span>
            <span class="text-14px">{{item.totalRevenue}}%</span>
          </div>
          <div class="flex-between mt-2">
            <span class="text-13px text-mid ">{{$t('defi.theTerm')}}</span>
            <span class="text-14px">{{item.profitTime}}{{$t('extra.hour')}}</span>
          </div>
          <div class="flex-between mt-2">
            <span class="text-13px text-mid ">{{$t('defi.coin')}}</span>
            <span class="text-14px">{{item.currency}}</span>
          </div>
          <div class="flex-between mt-2">
            <span class="text-13px text-mid ">{{$t('defi.minPledgeAmount')}}</span>
            <span class="text-14px">{{item.purchaseAmountMin}}</span>
          </div>
          <div class="px-1 mb-48">
            <div class="mid-btn w-full mt-4" v-if="item.remainingNumber<=0" disabled>{{$t('defi.soldOut')}}</div>
            <div class="mid-btn w-full mt-4" v-else @click="buy(item)">{{$t('defi.pledgeNow')}}</div>
          </div>
        </template>
      </div>
    </div>
