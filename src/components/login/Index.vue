<template>
  <div>
    ...
    <h4 class="section title">
      ...
      <img :src="require('@/assets/images/newlogo.png')" style="object-fit: contain;height:40px"/>
<!--      <span>AVIVA</span>-->
    </h4>
    ...
    <van-form :show-error="false" @submit="onSubmit">
      ...
      />
      <van-field
        v-model.trim="form.googleCaptcha"
        :placeholder="$t('login.enterGoogleCaptcha')"
        clearable
      />
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
      this.$http.post('user/login', this.form).then(async (data) => {
        this.form = {
          phone: null,
          password: null
        }
        ...
        if(this.userInfo && this.userInfo.payPwdStatus==0){
          this.$router.replace('/security/payPassword')
        }else{
          this.$router.replace('/')
        }
        // this.$ws.load()
