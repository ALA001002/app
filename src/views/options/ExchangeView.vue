<template>
  <div>
    <van-tabs type="card" v-model="tabActive" animated show-header no-padding bar-padding sticky>
      <van-tab :title="$t('extra.trendChart')" title-active-color="#07c160" title-inactive-color="#fff">
        <div class="flex-y-full" :style="{height}">
          ...
          <div class="section" :style="{paddingBottom: paddingBottom}">
            <div class="big-btn section" @click="tabActive=1">{{$t('spot.name')}}</div>
          </div>
        </div>
      </van-tab>
      <!--  旧版现货    -->
      ...
      <!--  新版现货    -->
      <van-tab :title="$t('spot.name')" background="#07c160" title-active-color="#07c160" title-inactive-color="#fff">
        <Spot @showBar="showBar=true" :code="data && data.symbolCode"></Spot>
      </van-tab>
            <van-tab :title="$t('app.options')">
                 <TimeContract :data="data"></TimeContract>
              <!-- &lt;!&ndash;        <headerVue @unitChange="unitChange" :unit="unit" :data="data" :active="tabActive" @showBar="showBar=true" @toRule="toRule"></headerVue>&ndash;&gt;
               ...
               &lt;!&ndash;        <Records :symbol="currentCoin" :unit="unit"></Records>&ndash;&gt;-->
               </van-tab>
...
.van-tabs__nav--card .van-tab {
  color: #07c160;
  border-right: 0.05333rem solid #07c160;
}
.big-btn {
  width: 100%;
  height: 2.66667rem;
