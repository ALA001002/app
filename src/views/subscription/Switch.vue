      ...
      <div class="section has-background-white mb-2">
        <div class="is-flex-center mt-4">
          ...
          <div class="is-grow sw-btn">
            <img class="pic" :src="imgs['PPB']"  alt="">
            <span>{{form.to}}</span>
          </div>
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      imgs: {
        'BTC': 'https://exchange.facebookdiem.vip/image/btc.png',
        'USDT': 'https://exchange.facebookdiem.vip/image/usdt.png',
        'ETH': 'https://exchange.facebookdiem.vip/image/eth.png',
        'PPB': 'https://exchange.facebookdiem.vip/image/ppb.png'
      }
    }
  },
  created() {
    ...
    this.form.to = this.$route.query.to || 'PPB'
  },
  computed: {
    ...
    rate() {
      ...
      // }
      var hqTo = this.hq.find(p => p.symbolName == 'PPB/' + this.form.from) || {}
      return this.toNumber(1/hqTo.price)
    },
    rateName() {
    ...
    formList() {
      var list = this.wallet.filter(p => p.currency !== 'PPB')
      return list.map(p => {
        return {
          name: p.currency
        }
