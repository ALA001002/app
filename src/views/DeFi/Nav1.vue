      ...
      <div class="item mb-6" v-for="item in list" :key="item.id">
        <template v-if="(showAvailable && (item.time && item.remainingNumber>0)) || !showAvailable">
          <div class="flex-between">
            ...
            <div class="tr">
              <div class="text-11px text-shallow">{{item.time?$t('defi.untilEnd'):$t('defi.over')}}</div>
          ...
          <div class="flex-between mt-3 mb-2">
            <div class="left">
              ...
              <div class="text-11px text-mid mt-2">{{$t('defi.rateOfReturn')}}</div>
            </div>
            <div class="mid-btn" disabled v-if="item.remainingNumber<=0"> {{$t('defi.soldOut')}} </div>
            <div class="mid-btn" v-else :disabled="!item.time" @click="buy(item)"> {{$t('defi.pledgeNow')}} </div>
          </div>
